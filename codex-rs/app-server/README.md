...
- `imageView` — `{id, path}` emitted when the agent invokes the image viewer tool in a live turn stream.
...
History/resume caveat: thread history is intentionally lossy. `imageView` items from live streams may be absent when reconstructing from persisted session history.
